...
def handle_413(e):
    ...
    flash("Uploaded file is too large (maximum 5 MB).", "danger")
...
def employee_management():
    ...
    return render_template(
        "admin/employee_management.html", full_name=full_name, employees=employees
    )


@app.route("/api/employee_of_month", methods=["GET"])
def api_employee_of_month():
    """
    Returns the active employee with the most hours worked in the current
    calendar month, along with their stats (total hours, days worked,
    average hours/day, total earnings).  Also returns the runner-up name
    and the current month label so the frontend can display context.
    ...
    try:
        cur.execute(
            ...
            SELECT
                ...
                ROUND(SUM(
                    CASE
                        WHEN a.hours_worked IS NOT NULL AND a.hours_worked > 0
                             THEN a.hours_worked
                        WHEN a.clock_in IS NOT NULL AND a.clock_out IS NOT NULL
                             THEN TIMESTAMPDIFF(SECOND, a.clock_in, a.clock_out) / 3600.0
                        ELSE 0
                    END
                ), 2)                                                        AS total_hours,
                ROUND(SUM(
                    CASE
                        ...
                        ELSE 0
                    END
                ) * COALESCE(e.hourly_rate, 0), 2)                          AS total_earnings
            FROM employees e
        ...
        days = int(winner["days_worked"] or 0)
        total_hours = float(winner["total_hours"] or 0)
        avg_hours = round(total_hours / days, 2) if days > 0 else 0
        earnings = float(winner["total_earnings"] or 0)
...
def api_shifts_get():
    """Return all configured shifts from the shift_config table."""
    # Allow any logged-in user (admin, manager, OR cashier) to read shifts.
    # Cashiers need this to populate the shift picker when clocking in/out.
...
@csrf.exempt
def api_shifts_post():
    """Replace all shifts with the submitted list (full replace strategy)."""
    if session.get("role") not in ["admin", "manager"]:
        return jsonify({"success": False, "message": "Unauthorized"}), 401
    ...
    conn.commit()
    cur.close()
    return jsonify({"success": True, "message": "Shifts saved."})
...
def product_management():
    ...
    return render_template("admin/product_management.html", full_name=full_name)
...
def run_auto_migration():
    ...
    # ── STEP 17: overtime_requests table ─────────────────────────────────────
    try:
        _ensure_overtime_requests_table()
    except Exception as exc:
    ...
    try:
        cur.execute(
            ...
            SELECT
                ...
                ROUND(COALESCE(SUM(CASE WHEN COALESCE(a.deduction_waived,0)=0
                               THEN COALESCE(a.late_deduction,0) ELSE 0 END),0)
...
def inventory():
    ...
    full_name = session.get("full_name") or "Admin"
    return render_template("admin/inventory.html", full_name=full_name)
...
# ── POST /api/inv_items/consume ──────────────────────────────
# Body: { id, used_qty, damaged_qty, note }
# Deducts (used_qty + damaged_qty) from stock, logs two separate rows.


@app.route("/api/inv_items/consume", methods=["POST"])
@csrf.exempt
def api_inv_items_consume():
    """
    Record a consumption event for an ingredient or supply.
    Splits the total deduction into 'used' and 'damaged' portions,
    writing a separate inv_log row for each so the history stays clean.

    Body JSON:
        id          – inv_items.id  (required)
        used_qty    – float >= 0    (units actually used/consumed)
        damaged_qty – float >= 0    (units wasted/damaged)
    ...
    try:
        ...
        if used_qty > 0:
            _log_inv_change(
                ...
                stock_after=new_stock,
                source="manual",
                note=f"[Used] {note}" if note else "[Used] Consumed",
                created_by=created_by,
            )

        # Log the 'damaged' portion
        if damaged_qty > 0:
            _log_inv_change(
                cur,
                item_id=item["id"],
...
# ── GET /api/inv_items/eod-report ────────────────────────────────────────────
# Returns the End-of-Day inventory report for a given date.
# For every active inv_item, it reconstructs:
#   beginning_stock = current_stock
#                     + all outflows that day (used, damaged, sale deductions)
#                     - all inflows that day (restocks, manual +adjustments)
#   consumed_used    = sum of |delta| for '[Used]' log rows on that day
#   consumed_damaged = sum of |delta| for '[Damaged]' log rows on that day
#   consumed_sale    = sum of |delta| for sale-source log rows on that day
#   restocked        = sum of delta for positive log rows on that day
#   ending_stock     = current_stock (from inv_items.stock — live value)
#
# Query param: date=YYYY-MM-DD  (defaults to today)


@app.route("/api/inv_items/eod-report", methods=["GET"])
def api_inv_items_eod_report():
    """
    End-of-Day inventory summary report.

    Returns one row per active inventory item with:
      item_id, name, type, unit, reorder_point,
      beginning_stock,
      consumed_used, consumed_damaged, consumed_sale,
      total_consumed,
      restocked,
      other_adjustments,
      ending_stock,
    ...
    try:
        ...
        for row in log_rows:
            ...
            if delta < 0:
                abs_d = abs(delta)
                if "[used]" in note:
                    buckets[iid]["used"] += abs_d
                elif "[damaged]" in note:
                    buckets[iid]["damaged"] += abs_d
                elif src == "sale":
...
# ── GET /api/sales/monthly-product-summary ────────────────────────────────────
# Full per-product sales summary for a given calendar month.
# Returns every product sold that month ranked by units, with best-seller /
# barely-sold classification and a month-over-month comparison row.
#
# Query params:
#   month  — YYYY-MM  (defaults to current month)
#   limit  — max products to return (default 100, max 200)


@app.route("/api/sales/monthly-product-summary", methods=["GET"])
def api_sales_monthly_product_summary():
    """
    Monthly product sales analytics.
    ...
    Response shape:
    {
      ...
      "summary": {
        ...
        "total_products": 22,
        "best_seller_threshold": 30,   // top-20% cutoff by units
        "barely_sold_threshold": 5     // bottom-20% cutoff by units
      },
      "products": [
        {
          "rank": 1,
          ...
          "revenue": 6000.00,
          "avg_price": 50.00,
          "revenue_share": 32.4,        // % of month revenue
          "units_share": 26.7,          // % of month units
          "tier": "best_seller",        // best_seller | mid | barely_sold
          "prev_units": 95,             // same product last month (null if none)
          "units_change_pct": 26.3      // MoM % change (null if no prev data)
        },
        ...
      ]
    }
    ...
    # Previous month boundaries (for MoM comparison)
    if month == 1:
        prev_year, prev_month = year - 1, 12
    else:
        prev_year, prev_month = year, month - 1
    prev_start = f"{prev_year:04d}-{prev_month:02d}-01"
    prev_end = month_start  # exclusive upper bound = this month's start
    ...
    try:
        ...
        # Sort units for percentile thresholds
        sorted_units = sorted([int(r["units_sold"]) for r in current_rows])
        p80_idx = max(0, int(n * 0.80) - 1)  # top-20% threshold
        p20_idx = max(0, int(n * 0.20) - 1)  # bottom-20% threshold
        ...
        for rank, r in enumerate(current_rows, 1):
            ...
            if prev is not None and prev > 0:
                ...
                units_change_pct = None  # can't divide by zero

            # Tier classification
            if units >= best_seller_threshold and n >= 3:
                tier = "best_seller"
            elif units <= barely_sold_threshold and n >= 3:
                tier = "barely_sold"
            else:
                tier = "mid"
...
def api_attendance_export():
    """
    Stream a CSV download of attendance records.
    Query params: start (YYYY-MM-DD), end (YYYY-MM-DD) — both optional.
    Filename: attendance_YYYYMMDD_YYYYMMDD.csv
    """
    if session.get("role") not in ["admin", "manager"]:
        return jsonify({"success": False, "message": "Unauthorized"}), 401
    ...
    try:
        ...
        if start_date and end_date:
            ...
            where.append("DATE(a.clock_in) >= %s")
            params.append(start_date)
        elif end_date:
            where.append("DATE(a.clock_in) <= %s")
            params.append(end_date)
        ...
        app.logger.error(f"[attendance] export CSV error: {exc}")
        return jsonify({"success": False, "message": str(exc)}), 500
...
def api_payroll_export():
    """
    Stream a CSV download of payroll period records.
    Query params: start (YYYY-MM-DD), end (YYYY-MM-DD) — both optional.
    Filename: payroll_YYYYMMDD_YYYYMMDD.csv
    """
    if not is_admin():
        return jsonify({"success": False, "message": "Unauthorized"}), 401
    ...
    try:
        ...
        if start_date and end_date:
            ...
            where.append("pp.period_start >= %s")
            params.append(start_date)
        elif end_date:
            where.append("pp.period_end <= %s")
            params.append(end_date)
        ...
        app.logger.error(f"[payroll] export CSV error: {exc}")
        return jsonify({"success": False, "message": str(exc)}), 500
...
def _auto_clockout_missed(target_date=None):
    """
    Close any attendance rows from target_date (default: yesterday) that still
    have clock_in set but clock_out NULL.  The synthetic clock_out is set to
    23:59:59 of that day (end-of-day cutoff), so payroll hours are capped at
    the shift rather than leaking into the next day.

    Called at startup (to catch previous-day stragglers) and nightly at midnight
    ...
    try:
        ...
        cur.execute(
            """
            SELECT a.attendance_id, a.clock_in, a.employee_id,
                   COALESCE(e.hourly_rate, 0) AS hourly_rate
            FROM   attendance a
            JOIN   employees  e ON e.employee_id = a.employee_id
            WHERE  a.attendance_date = %s
        ...
        if not rows:
            cur.close()
            app.logger.info(f"[auto-clockout] No missed clock-outs for {target_date}.")
            return 0
        ...
        for row in rows:
            ...
            cur.execute(
                ...
                UPDATE attendance
                 ...
                 WHERE attendance_id = %s
                """,
                (clock_out, hours, rate, earnings, ps, pe, att_id),
            )
            app.logger.warning(
                f"[auto-clockout] att#{att_id} emp#{row['employee_id']} "
                f"auto-clocked-out at {clock_out} — {hours:.2f}h x P{rate:.2f} = P{earnings:.2f}"
            )
            count += 1
        ...
        app.logger.error(f"[auto-clockout] Failed for {target_date}: {exc}")
        return 0


def _midnight_clockout_thread():
    """
    Background daemon thread.  Sleeps until 00:00:30 of the next calendar day,
    runs _auto_clockout_missed() for the day that just ended, then sleeps again.
...
@csrf.exempt
def api_attendance_manual_clockout():
    """
    Admin/manager override: manually set clock_out for an attendance record.

    POST JSON:
        {
          "attendance_id": int,
          "clock_out": "HH:MM"   // time string on the record's own date
        }
    ...
    try:
        ...
        cur.execute(
            """
            SELECT a.attendance_id, a.clock_in, a.clock_out, a.attendance_date,
                   COALESCE(e.hourly_rate, 0) AS hourly_rate,
                   e.employee_id
            FROM   attendance a
            JOIN   employees  e ON e.employee_id = a.employee_id
        ...
        cur.execute(
            ...
            UPDATE attendance
             ...
             WHERE attendance_id = %s
            """,
            (clock_out_dt, hours, rate, earnings, ps, pe, att_id),
        )
        mysql.connection.commit()
        cur.close()

        actor = session.get("full_name") or session.get("username") or "admin"
        app.logger.info(
            f"[manual-clockout] att#{att_id} emp#{row['employee_id']} overridden by "
...
def _ensure_overtime_requests_table():
    ...
    try:
        ...
        try:
            ...
            pass  # safe to ignore — column already has the value or table is brand new
        cur.close()
        app.logger.info("[migration] overtime_requests table ensured")
    except Exception as exc:
        app.logger.error(f"[migration] _ensure_overtime_requests_table: {exc}")


@app.route("/api/overtime/request", methods=["POST"])
@csrf.exempt
def api_overtime_request_submit():
    """Cashier submits an overtime request."""
    if "employee_id" not in session or session.get("role") != "cashier":
        return jsonify({"success": False, "message": "Unauthorized"}), 401
    ...
    cur.execute(
        "SELECT request_id FROM overtime_requests WHERE employee_id=%s AND request_date=%s AND status='pending' LIMIT 1",
    ...
    attendance_id = att_row["attendance_id"] if att_row else None

    cur.execute(
        """INSERT INTO overtime_requests
           (employee_id, attendance_id, request_date, extended_hours, reason, status)
...
def api_overtime_my_requests():
    """Returns the logged-in cashier's own overtime requests."""
    if "employee_id" not in session or session.get("role") != "cashier":
        return jsonify({"success": False, "message": "Unauthorized"}), 401
    ...
    cur.execute(
        """SELECT request_id,
                  ...
                  extended_hours, reason, status, admin_note,
...
@app.route("/api/overtime/cancel/<int:request_id>", methods=["POST"])
@csrf.exempt
def api_overtime_cancel(request_id):
    """Cashier cancels their own pending overtime request."""
    if "employee_id" not in session or session.get("role") != "cashier":
        return jsonify({"success": False, "message": "Unauthorized"}), 401
    ...
    if req_row["employee_id"] != employee_id:
        cur.close()
        return jsonify({"success": False, "message": "Unauthorized"}), 403
...
def api_overtime_requests_list():
    """Admin/Manager: list all overtime requests, optionally filtered by status."""
    if session.get("role") not in ["admin", "manager"]:
        return jsonify({"success": False, "message": "Unauthorized"}), 401
...
@app.route("/api/overtime/review/<int:request_id>", methods=["POST"])
@csrf.exempt
def api_overtime_review(request_id):
    """Admin/Manager approves or denies an overtime request."""
    if session.get("role") not in ["admin", "manager"]:
        return jsonify({"success": False, "message": "Unauthorized"}), 401
