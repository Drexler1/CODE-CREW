    ...
    try:
        ...
        if cur.fetchone()['cnt'] == 0:
            ...
            samples = [
                ...
                (cat_map.get('Stationery'), 'Fountain Pen',        'Premium writing instrument',             'ST-003',1200.00, 500.00,  7, 3, 'pcs'),
        ...
        cur.execute("""
            INSERT INTO products
                (category_id, name, description, sku, price, cost,
                 stock, reorder_point, unit, image_url)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """, (category_id, name, description, sku, price, cost,
              stock, reorder_point, unit, image_url))
        mysql.connection.commit()
...
def api_inventory_items_update(product_id):
    ...
    try:
        ...
        """, (category_id, name, description, sku, price, cost,
              stock, reorder_point, unit, image_url, product_id))
        mysql.connection.commit()
        affected = cur.rowcount
        cur.close()
        if affected == 0:
