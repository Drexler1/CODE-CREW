    ...
    try:
        ...
        cur.execute(
            """INSERT INTO employees
               (full_name, username, username_hash, password, password_hash, role, contact_number, employment_status)
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""",
            (aes_encrypt(full_name), aes_encrypt(username), aes_username_hash(username),
             aes_encrypt(password), hash_password(password), role, aes_encrypt(contact), status)
        )
...
def log_attendance():
    ...
    if not token_data:
        app.logger.warning(
            f"[log_attendance] REJECTED — no verify_token for employee {employee_id_str} "
    ...
    if token_data['employee_id'] != employee_id_str:
        app.logger.warning(
            f"[log_attendance] TOKEN MISMATCH — token for employee "
            f"{token_data['employee_id']} used by employee {employee_id_str} "
            f"ip={request.remote_addr}"
        )
        # Log as a mismatch attempt
        try:
            mlog_cur = mysql.connection.cursor()
            mlog_cur.execute(
                """INSERT INTO face_mismatch_log
