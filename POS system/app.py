...
def api_products_pos():
    ...
    try:
        ...
        cur.close()
        return jsonify({"success": True, "items": items, "products": items, "categories": categories})
    except Exception as exc:
...
def api_payroll_history():
    ...
    for row in rows:
        ...
        row["finalized_at"] = str(row["finalized_at"]) if row.get("finalized_at") else None
...
def api_pos_checkout():
    ...
    try:
        ...
        if discount_type_str in ("senior", "pwd"):
            ...
            discount_amount = min(round(float(data.get("discount_amount") or 0), 2), subtotal)
        ...
        for li in line_items:
            ...
            new_stock = float(updated_row["stock"]) if updated_row else max(0, float(cup_row["stock"]) - qty)
...
def api_inventory_items():
    ...
    try:
        ...
        return jsonify({"success": True, "items": items, "products": items, "total": len(items)})
    except Exception as exc:
...
def api_inventory_bulk_action():
    ...
    if not action or not product_ids:
        return (
            jsonify({"success": False, "message": "action and product_ids are required"}),
            400,
        )
    ...
    try:
        ...
        if action == "delete":
            ...
            try:
                ...
                cur.close()
                return jsonify({"success": False, "message": "value (integer delta) required for restock"}), 400
            ...
            try:
                ...
                cur.close()
                return jsonify({"success": False, "message": "value (integer threshold) required for set_reorder"}), 400
        ...
        app.logger.info(f"[inventory] bulk-action={action} ids={product_ids} affected={affected}")
            ...
            history.append(
                {
                    ...
                    "stock_after": float(row["stock_after"]) if row["stock_after"] is not None else None,
                    "reference": ref,
