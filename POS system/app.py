...
def verify_face():
    ...
    if _srole in ('cashier', 'manager'):
        ...
        if _admin_id:
            ...
            _ca.execute("SELECT username_hash FROM admins WHERE admin_id=%s", (_admin_id,))
            _adm = _ca.fetchone()
            if _adm and _adm.get('username_hash'):
                _ca.execute("SELECT employee_id FROM employees WHERE username_hash=%s LIMIT 1",
                             (_adm['username_hash'],))
...
def dashboard():
    ...
    try:
        ...
        if yest_count > 0:
            transaction_change = round((today_count_raw - yest_count) / yest_count * 100, 1)
...
def api_products_upload_image():
    ...
    if not _allowed_image(file.filename):
        return jsonify({'success': False, 'message': 'File type not allowed. Use JPG, PNG, WEBP or GIF.'}), 400
    ...
    if size_bytes > MAX_PRODUCT_IMAGE_MB * 1024 * 1024:
        return jsonify({'success': False, 'message': f'Image too large (max {MAX_PRODUCT_IMAGE_MB} MB)'}), 400
    ...
    try:
        ...
        if product_id:
            try:
                cur2 = mysql.connection.cursor(DictCursor)
                cur2.execute("SELECT image_url FROM products WHERE product_id=%s", (int(product_id),))
                row = cur2.fetchone()
                cur2.close()
...
def api_products_pos():
    ...
    try:
        ...
        # All active products
        cur.execute("""
            SELECT p.product_id, p.name, p.description, p.icon,
                   p.image_url, p.price, p.stock, p.unit,
...
def api_inventory_categories_update(category_id):
    """Update an existing category's name and icon."""
    if not is_admin():
        return jsonify({'success': False, 'message': 'Unauthorized'}), 401
    ...
    try:
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE categories SET name=%s, icon=%s WHERE category_id=%s",
...
def api_my_attendance():
    ...
    if range_start and range_end:
        try:
            ...
            cur.close()
            return jsonify({'success': False, 'message': 'Invalid date format'}), 400
        cur.execute(BASE_SQL + " AND a.attendance_date BETWEEN %s AND %s ORDER BY a.attendance_date DESC, a.clock_in DESC",
                    (employee_id, range_start, range_end))
        rows = cur.fetchall()
        cur.close()
        total_hours = round(sum(float(r['hours_worked']) for r in rows if r.get('hours_worked')), 2)
        return jsonify({'success': True, 'records': rows, 'total_hours': total_hours,
        ...
        if date_str:
            try: dt = datetime.strptime(date_str, "%Y-%m-%d").date()
                ...
                cur.close()
                return jsonify({'success': False, 'message': 'Invalid date format'}), 400
        cur.execute(BASE_SQL + " AND a.attendance_date = %s ORDER BY a.clock_in DESC", (employee_id, dt))
        rows = cur.fetchall()
        cur.close()
        total_hours = round(sum(float(r['hours_worked']) for r in rows if r.get('hours_worked')), 2)
        return jsonify({'success': True, 'date': str(dt), 'records': rows, 'total_hours': total_hours})
...
def run_auto_migration():
    ...
    try:
        ...
        app.logger.error(f"[migration] Step 6 (lockout table) failed (non-fatal): {exc}")

    # ── STEP 7: widen face_model_path VARCHAR(255) → MEDIUMTEXT ──────────────
    try:
        _widen_face_model_path()
    except Exception as exc:
        app.logger.error(f"[migration] Step 7 (face_model_path) failed (non-fatal): {exc}")
    ...
    try:
        ...
        app.logger.error(f"[migration] Step 9 (payroll tables) failed (non-fatal): {exc}")
...
# ║                                                                              ║
# ║  Tables:                                                                     ║
# ║    categories  — product categories (id, name, icon, created_at)             ║
...
def _ensure_inventory_tables():
    ...
    try:
        ...
        if cur.fetchone()['cnt'] == 0:
            ...
            cur.executemany(
                "INSERT IGNORE INTO `categories` (name, icon) VALUES (%s, %s)",
                defaults
            )
            conn.commit()
        ...
        if cur.fetchone()['cnt'] == 0:
            ...
            samples = [
                (cat_map.get('Books'),      'The Great Gatsby',    'A classic novel by F. Scott Fitzgerald', 'BK-001', 450.00, 200.00, 12, 5, 'pcs', '📕'),
                (cat_map.get('Books'),      'Pride & Prejudice',   'Jane Austen timeless romance',           'BK-002', 520.00, 220.00, 10, 5, 'pcs', '📗'),
                (cat_map.get('Books'),      '1984 by Orwell',      'Dystopian social science fiction',       'BK-003', 480.00, 210.00, 14, 5, 'pcs', '📘'),
                (cat_map.get('Flowers'),    'Rose Bouquet',        'Fresh red roses, dozen',                 'FL-001', 850.00, 350.00,  8, 3, 'bunch','🌹'),
                (cat_map.get('Flowers'),    'Tulip Bunch',         'Seasonal tulips, mixed colors',          'FL-002', 680.00, 280.00,  4, 3, 'bunch','🌷'),
                (cat_map.get('Flowers'),    'Sunflower Bouquet',   'Bright sunflowers, half dozen',          'FL-003', 720.00, 300.00,  3, 3, 'bunch','🌻'),
                (cat_map.get('Café'),       'Cappuccino',          'Espresso with steamed milk foam',        'CF-001', 150.00,  40.00, 50, 10,'cup',  '☕'),
                (cat_map.get('Café'),       'Latte',               'Espresso with steamed milk',             'CF-002', 140.00,  38.00, 50, 10,'cup',  '☕'),
                (cat_map.get('Café'),       'Croissant',           'Buttery French pastry',                  'CF-003', 120.00,  45.00, 30, 10,'pcs',  '🥐'),
                (cat_map.get('Café'),       'Espresso',            'Double shot espresso',                   'CF-004', 100.00,  30.00,  0, 10,'cup',  '☕'),
                (cat_map.get('Stationery'), 'Notebook Set',        'A5 ruled notebooks, pack of 3',          'ST-001', 280.00, 100.00, 25, 5, 'set',  '📓'),
                (cat_map.get('Stationery'), 'Bookmark Set',        'Decorative bookmarks, pack of 5',        'ST-002', 150.00,  50.00, 30, 8, 'set',  '🔖'),
                (cat_map.get('Stationery'), 'Fountain Pen',        'Premium writing instrument',             'ST-003',1200.00, 500.00,  7, 3, 'pcs',  '🖊️'),
                (cat_map.get('Gifts'),      'Gift Basket',         'Assorted café and book combo',           'GF-001',1500.00, 600.00,  5, 2, 'pcs',  '🎁'),
                (cat_map.get('Gifts'),      'Book Light',          'LED clip-on reading light',              'GF-002', 450.00, 180.00, 15, 5, 'pcs',  '💡'),
            ]
            cur.executemany("""
                INSERT INTO `products`
                    (category_id, name, description, sku, price, cost, stock,
                     reorder_point, unit, icon)
...
def api_pos_checkout():
    ...
    try:
        ...
        for raw in items_raw:
            ...
            if prod['stock'] < qty:
                stock_errors.append(
                    f"'{prod['name']}' only has {prod['stock']} {prod['unit']}(s) left"
                )
                continue
        ...
        """, [(transaction_id, li['product_id'], li['product_name'],
               li['category_name'], li['unit_price'],
               li['quantity'], li['line_total']) for li in line_items])
        ...
        return jsonify({'success': True, 'transaction_id': transaction_id, 'receipt': receipt})
...
def api_pos_transactions():
    ...
    try:
        ...
        sql = f"""
            SELECT t.transaction_id, t.cashier_name, t.subtotal,
...
def api_pos_transaction_detail(transaction_id):
    ...
    try:
        cur = mysql.connection.cursor(DictCursor)
        cur.execute("""
            SELECT transaction_id, cashier_name, subtotal, discount_amount,
                   tax_amount, total_amount, amount_tendered, change_amount,
        ...
        if not is_admin():
            ...
            cur.execute(
                "SELECT cashier_id FROM transactions WHERE transaction_id=%s",
                (transaction_id,)
            )
            owner = cur.fetchone()
            if not owner or owner['cashier_id'] != cashier_id:
                cur.close()
...
def _get_low_stock_items(limit=20):
    """
    Return a list of dicts for products at or below their reorder point.
    Used by both the dashboard route and /api/inventory/low-stock.

    Each dict: { name, category, stock, unit, reorder_point,
                 status ('out'|'low'), status_class, status_label }
    """
    cur = mysql.connection.cursor(DictCursor)
    cur.execute("""
        SELECT p.name, p.stock, p.reorder_point, p.unit,
    ...
    for row in rows:
        ...
        items.append({
            ...
            'status_label':  'Critical' if is_out else 'Low',
        })
    return items
...
def api_inventory_categories():
    ...
    try:
        ...
        cur.close()
        return jsonify({'success': True, 'category_id': new_id, 'message': 'Category added'})
    except Exception as exc:
        app.logger.error(f"[inventory] categories POST: {exc}")
        return jsonify({'success': False, 'message': 'Category name already exists or DB error'}), 400
...
def api_inventory_items():
    ...
    try:
        ...
        sql = f"""
            SELECT p.product_id, p.name, p.description, p.sku, p.icon, p.image_url,
                   p.price, p.cost, p.stock, p.reorder_point, p.unit,
        ...
        cur.execute("""
            INSERT INTO products
                (category_id, name, description, sku, price, cost,
                 stock, reorder_point, unit, icon, image_url)
...
def api_inventory_items_update(product_id):
    ...
    try:
        ...
        """, (category_id, name, description, sku, price, cost,
              stock, reorder_point, unit, icon, image_url, product_id))
        mysql.connection.commit()
        affected = cur.rowcount
        cur.close()
        if affected == 0:
