...
import base64, cv2, os, numpy as np, time, hashlib
...
def aes_decrypt(token: str) -> str:
    """Decrypt AES-256-CBC token. Falls back to plain text for legacy rows."""
    if not token:
        return token
    try:
        ...
        return token   # legacy plain-text row — return as-is
...
def login():
    if request.method == 'POST':
        ...
        # ── Manager tab ──────────────────────────────────────────────────────
            ...
            if employee and employee.get('password','').strip() == password:
                session.clear()
            ...
            if admin and admin.get('password','').strip() == password:
                session.clear()
    ...
    try:
        ...
        cur.execute(
            """INSERT INTO employees
               (full_name, username, username_hash, password, role, contact_number, employment_status)
               VALUES (%s, %s, %s, %s, %s, %s, %s)""",
            (aes_encrypt(full_name), aes_encrypt(username), aes_username_hash(username),
             aes_encrypt(password), role, aes_encrypt(contact), status)
        )
        ...
        if embeddings and best_face is not None:
            ...
            cur.execute(
                """UPDATE employees
                   ...
                   WHERE employee_id=%s""",
                (enc_name, enc_username, u_hash, role, enc_contact, status, face_path, employee_id)
            )
            ...
            cur.execute(
                """UPDATE employees
                   SET full_name=%s, username=%s, username_hash=%s, role=%s,
                       contact_number=%s, employment_status=%s
                   WHERE employee_id=%s""",
                (enc_name, enc_username, u_hash, role, enc_contact, status, employee_id)
            )
...
def create_admin():
    ...
    if request.method == 'POST':
        ...
        # ── 4. Insert new admin (AES-256 encrypted) ──────────────────────────
        try:
            cur.execute(
                "INSERT INTO admins (full_name, username, username_hash, password) VALUES (%s, %s, %s, %s)",
                (aes_encrypt(full_name), aes_encrypt(username), aes_username_hash(username), aes_encrypt(password))
            )
            mysql.connection.commit()
            cur.close()
...
def run_auto_migration():
    """
    Ensure username_hash columns exist and all credential rows are AES-encrypted.
    Called once at startup via before_request guard below.
    """
    try:
        conn = mysql.connection
        cur  = conn.cursor(DictCursor)

        # ── 1. Add username_hash column where missing ────────────────────────
        for tbl in ('employees', 'admins'):
            try:
                ...
                conn.commit()
                app.logger.info(f"[auto-migration] Added username_hash to {tbl}")
            except Exception:
                pass   # Column already exists — that's fine

        # ── 2. Encrypt plaintext employees rows ──────────────────────────────
        cur.execute(
        ...
        for row in cur.fetchall():
            ...
            for f in ('username', 'full_name', 'password', 'contact_number'):
                v = row.get(f)
                if v and not _is_enc(str(v)):
                    upd[f] = aes_encrypt(str(v).strip())
            raw_u = row.get('username', '')
            if raw_u and not _is_enc(raw_u):
            ...
            if upd:
                ...
                app.logger.info(
                    f"[auto-migration] Encrypted employee #{row['employee_id']}"
                )

        # ── 3. Encrypt plaintext admins rows ─────────────────────────────────
        ...
        for row in cur.fetchall():
            ...
            if upd:
                ...
                app.logger.info(
                    f"[auto-migration] Encrypted admin #{row['admin_id']}"
                )
