...
# ================= FACE TRAINING =================
def train_face_model(image_path, model_path, employee_id):
...
def reset_liveness(employee_id):
    liveness_sessions[employee_id] = {
        "movement": False,
        "previous_x": None,
...
def verify_face():
    ...
    session_state = liveness_sessions[employee_id]

    if (datetime.now() - session_state["started_at"]).seconds > 15:
        reset_liveness(employee_id)
        session_state = liveness_sessions[employee_id]
    ...
    if not os.path.exists(model_path):
        return jsonify({'success': False, 'message': 'Face model missing'})
    ...
    for (x, y, w, h) in faces:
        ...
        # ---------- HEAD MOVEMENT ----------
        if session_state["previous_x"] is not None:
            movement = abs(x - session_state["previous_x"])

            if movement > 20:
                session_state["movement"] = True

        session_state["previous_x"] = x

        if not session_state["movement"]:
            return jsonify({'success': False, 'message': 'Move your head slightly'})
        ...
        if confidence < 55:
            ...
            return jsonify({
                'success': False,
                'message': 'Face does not match employee',
                'confidence': float(confidence)
            })
...
def add_employee():
    ...
    try:
        ...
        # 1️⃣ Insert employee first (without face data)
        cur.execute("""
        ...
        if face_file:
            ...
            model_path = os.path.join(MODEL_FOLDER, f"{username}.yml")

            # 3️⃣ Train face model using employee_id
            if not train_face_model(image_path, model_path, employee_id):
            ...
            # 4️⃣ Update employee with face paths
            cur.execute("""
                UPDATE employees
                SET face_image_path=%s,
                    face_model_path=%s
                WHERE employee_id=%s
            """, (face_image_path, face_model_path, employee_id))
...
def update_employee(employee_id):
    ...
    if face_file:
        ...
        if not train_face_model(image_path, model_full_path, employee_id):
            return jsonify({'success': False, 'message': 'No face detected'})
